...
sys.path.append("C:\Shared\polychrom-shared") # update with path to polychrom repo, if not already in your python path
import polychrom
...
# general parameters 
trajectoryLength = parValues[10] # time duration of simulation 
density = parValues[11] #  0.2  # fraction of the volume of the sphere which is occupied by the polymer (or effective excluded volume, if using periodic boundary conditions)
  
   
#  ==========Extrusion sim parameters====================
# there is probably a more elegant way to read in text values than ast.literal_eval, but this works.  
GPU_ID = str(parValues[14]) # this should be a string -- which GPU to run on, eg "0", or "1" 
repulsionEnergy= parValues[15] # this is just a number
N1 = parValues[0] # Number of monomers in the polymer/chromosome
M = parValues[1]  # Number of chromosomes in the simulation
num_chains = M  # same as above, multiple variable names were introduced.
N = N1 * M # total number of monomers in the full simulation 
LIFETIME = parValues[2]  #  extruder lifetime
SEPARATION = parValues[3] # separation between extruders in monomer units (extruder density) 
ctcfSites = np.array(ast.literal_eval(parValues[4])) # CTCF site locations, in monomer units
ctcfDir = np.array(ast.literal_eval(parValues[13])) # CTCF site direction 
...
interactionMatrix = np.array(ast.literal_eval(parValues[7]))  # interaction strength between different compartments 
saveFolder = parValues[8]   # place to save the data
